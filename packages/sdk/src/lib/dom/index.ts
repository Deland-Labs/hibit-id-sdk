...
export class HibitIdController {
  ...
  private handleMouseMove = (ev: MouseEvent) => {
    ...
    if (this.dragging) {
      ...
      const right = clamp(
        0,
        window.innerWidth - rect.right - ev.movementX,
        window.innerWidth - rect.width
      );
      const bottom = clamp(
        0,
        window.innerHeight - rect.bottom - ev.movementY,
  ...
  private handleTouchMove = (ev: TouchEvent) => {
    ...
    if (this.dragging) {
      ...
      const right = clamp(
        0,
        window.innerWidth - rect.right - movement.x,
        window.innerWidth - rect.width
      );
      const bottom = clamp(
        0,
        window.innerHeight - rect.bottom - movement.y,
        window.innerHeight - rect.height
      );
      this.container.style.right = `${right}px`;
      this.container.style.bottom = `${bottom}px`;
...
export class HibitIdIframe {
  ...
  constructor(
    ...
    if (existed) {
      ...
      this.iframe = this._container.querySelector(
        'iframe'
