...
class Keypair {
  ...
  public static random(): Keypair {
    ...
    const publicKey = base.toHex(signUtil.secp256k1.publicKeyCreate(privateKeyBytes, true));
    const xOnlyPublicKey = base.toHex(signUtil.schnorr.secp256k1.schnorr.getPublicKey(privateKeyBytes));
  ...
  public static fromPrivateKeyHex(key: string): Keypair {
    return new Keypair(
      ...
      base.toHex(signUtil.schnorr.secp256k1.schnorr.getPublicKey(base.fromHex(key)))
    );
  }
  ...
  public static fromPublicKeyHex(pubKey: string): Keypair {
    ...
    const x = signUtil.schnorr.secp256k1.schnorr.utils.bytesToNumberBE(base.fromHex(pubKey).slice(1, 33));
  ...
  public signMessageWithAuxData(message: Uint8Array, auxData32: Uint8Array): Uint8Array {
    ...
    const hash = base.blake2(Buffer.from(message), 256, Blake2bHashKey.PersonalMessageSigning);
    return signUtil.schnorr.secp256k1.schnorr.sign(hash, base.toHex(base.fromHex(this.privateKey)), auxData32);
  }
  ...
  public verifyMessage(signature: Uint8Array, message: Uint8Array): boolean {
    const hashedMsg = base.blake2(Buffer.from(message), 256, Blake2bHashKey.PersonalMessageSigning);
    return signUtil.schnorr.secp256k1.schnorr.verify(signature, hashedMsg, this.xOnlyPublicKey!);
  }
}
