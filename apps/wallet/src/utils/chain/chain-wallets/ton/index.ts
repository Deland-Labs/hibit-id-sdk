...
export class TonChainWallet extends BaseChainWallet {
  ...
  public override balanceOf = async (address: string, assetInfo: AssetInfo) => {
    ...
    if (assetInfo.chainAssetType.equals(ChainAssetType.Jetton)) {
      ...
      return new BigNumber(String(balance)).shiftedBy(
        -assetInfo.decimalPlaces.value
      );
    }

    throw new Error(
      `Ton: unsupported chain asset type ${assetInfo.chainAssetType.toString()}`
    );
  };
  ...
  public override transfer = async (
      ...
      await this.wallet!.sendTransfer({
        ...
        messages: [
          internal({
            ...
            bounce: this.seeIfBounceable(toAddress)
          })
        ],
    ...
    if (assetInfo.chainAssetType.equals(ChainAssetType.Jetton)) {
      ...
      if (gasBn.lt(minGas)) {
        throw new Error(
          `Insufficient gas balance (at least ${minGas.toString()} Ton)`
        );
      }
      ...
      try {
        ...
        if (e.response?.status === 500) {
          ...
          if (
            /^LITE_SERVER_UNKNOWN:[.\s\S]*inbound external message rejected by transaction[.\s\S]*$/i.test(
              message
            )
          ) {
            throw new Error(
              `Insufficient gas balance (at least ${minGas} Ton)`
            );
          }
        }
        throw e;
  ...
  private createTonConnectTransfer = async (
    seqno: number,
    payload: TonConnectTransactionPayload
  ) => {
    await this.readyPromise;
