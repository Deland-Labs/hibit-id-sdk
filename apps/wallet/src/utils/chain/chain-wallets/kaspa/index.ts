...
export class KaspaChainWallet extends BaseChainWallet {
  ...
  public override balanceOf = async (address: string, assetInfo: AssetInfo) => {
    ...
    if (assetInfo.chainAssetType.equals(ChainAssetType.KRC20)) {
      const chainInfo = getChainByChainId(
        new ChainId(assetInfo.chain, assetInfo.chainNetwork)
      );
      if (!chainInfo) {
        throw new Error(
          `Kaspa: unsupported asset chain ${assetInfo.chain.toString()}_${assetInfo.chainNetwork.toString()}`
        );
      }
  ...
  public override transfer = async (
    ...
    try {
      // native
      if (assetInfo.chainAssetType.equals(ChainAssetType.Native)) {
        const sendParam = new SendKasPramas(
      ...
      if (assetInfo.chainAssetType.equals(ChainAssetType.KRC20)) {
        ...
        const sendKrc20Param = new SendKrc20Pramas(
          this.keyPair.toAddress(this.networkId.networkType),
          BigInt(amount.shiftedBy(assetInfo.decimalPlaces.value).toString()),
          toAddress,
          assetInfo.contractAddress,
  ...
  public override getEstimatedFee = async (
    ...
    // native
    if (assetInfo.chainAssetType.equals(ChainAssetType.Native)) {
      const sendKasParam = new SendKasPramas(
        this.keyPair.toAddress(this.networkId.networkType),
        BigInt(amount.shiftedBy(assetInfo.decimalPlaces.value).toString()),
        Address.fromString(toAddress),
        this.networkId
  ...
  private createTransactionsByOutputs = async (
    sendParam: SendKasPramas | SendKrc20Pramas,
    comitTxId?: string
  ): Promise<{
    priorityFee: Fees;
    result: ReturnType<typeof createTransactions>;
  }> => {
    const isKrc20Tx = sendParam instanceof SendKrc20Pramas;
    const isReveal = comitTxId !== undefined;
    ...
    const settings: GeneratorSettings = !isKrc20Tx
      ? (sendParam as SendKasPramas).toGeneratorSettings(utxoEntries)
      : !isReveal
        ? (sendParam as SendKrc20Pramas).toCommitTxGeneratorSettings(
            utxoEntries
          )
        : (sendParam as SendKrc20Pramas).toRevealTxGeneratorSettings(
            utxoEntries,
            Hash.fromHex(comitTxId)
          );
